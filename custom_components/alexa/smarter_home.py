...
class _AlexaPercentageController(_AlexaInterface):
    ...
    https://developer.amazon.com/docs//evice-apis/alexa-percentagecontroller.html
    """
...
class _AlexaChannelController(_AlexaInterface):
...
class _MediaPlayerCapabilities(_AlexaEntity):
    ...
    def interfaces(self):
        ...
        if supported & media_player.SUPPORT_PLAY_MEDIA:
            yield _AlexaChannelController(self.entity)
...
async def async_api_changechannel(hass, config, directive, context):
    ...
    data = {
        ...
        media_player.ATTR_MEDIA_CONTENT_TYPE: 'channel'
    }

    await hass.services.async_call(
        entity.domain, media_player.SERVICE_PLAY_MEDIA,
...
async def async_api_skipchannel(hass, config, directive, context):
    ...
    if channel > 0:
        await hass.services.async_call(
          entity.domain, media_player.SERVICE_MEDIA_NEXT_TRACK,
          data, blocking=False, context=context)
    if channel < 0:
        await hass.services.async_call(
          entity.domain, media_player.SERVICE_MEDIA_PREVIOUS_TRACK,
          data, blocking=False, context=context)
...
async def async_api_adjust_volume(hass, config, directive, context):
    ...
    _LOGGER.debug("Received Alexa speaker adustvolume   adjust volume: %s", volume_delta)
...
async def async_api_adjust_volume_step(hass, config, directive, context):
    ...
    #the if /elif are not really used so fall through to the last. left in for testing later
    #if volume_int == 1:
     #   await hass.services.async_call(
    ...
    if volume_int != 0:
        data = {
            ATTR_ENTITY_ID: entity.entity_id,
            media_player.ATTR_MEDIA_VOLUME_LEVEL: volume_step
        }
        await hass.services.async_call(
            entity.domain, media_player.SERVICE_VOLUME_SET,
            data, blocking=False, context=context)
...
async def async_api_set_volume(hass, config, directive, context):
    ...
    entity = directive.entity
    _LOGGER.debug("Received Alexa speaker setvolume  adjust volume: %s", volume)
